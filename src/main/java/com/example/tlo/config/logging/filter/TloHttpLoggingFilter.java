package com.example.tlo.config.logging.filter;


import com.example.tlo.api.common.CommonResponseDTO;
import com.example.tlo.common.util.useragent.UserAgentUtil;
import com.example.tlo.config.logging.ReusableRequestWrapper;
import com.example.tlo.config.logging.TloLoggerMessage;
import com.example.tlo.config.logging.TloLoggerWriter;
import com.example.tlo.config.logging.TloThreadLocalComponent;
...
public class TloHttpLoggingFilter extends OncePerRequestFilter {
    ...
    public TloLoggerMessage.TloLoggerMessageBuilder tslBuilder(HttpServletRequestWrapper requestWrapper) {
        ...
        UserDevice userDevice = UserAgentUtil.getDeviceInfo(requestWrapper.getHeader("user-agent"), userAgentProperties.getDevices());
        String devInfo = null;
        String osInfo = null;
        String modelInfo = null;
        if (userDevice != null) {
            devInfo = userDevice.getDevice();
            osInfo = userDevice.getOs();
            modelInfo = userDevice.getModel();
        }
            ...
            if (StringUtils.isNotEmpty(responseWrapper.getContentType()) && responseWrapper.getContentType().contains(MediaType.APPLICATION_JSON_VALUE)) {
                ...
                if (buf.length > 0) {
                    try {
                        ...
                        CommonResponseDTO result = objectMapper.readValue(payload, CommonResponseDTO.class);
